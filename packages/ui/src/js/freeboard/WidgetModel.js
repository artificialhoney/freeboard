export function WidgetModel(theFreeboardModel, widgetPlugins) {
  ...
  this.settings.subscribe(function (newValue) {
    if (
      ...
      _.isFunction(self.widgetInstance.onSettingsChanged)
    ) {
  ...
  this.processDatasourceUpdate = function (datasourceName) {
    var refreshSettingNames =
      self.datasourceRefreshNotifications[datasourceName];
  ...
  this.processSizeChange = function () {
    if (
      !_.isUndefined(self.widgetInstance) &&
      _.isFunction(self.widgetInstance.onSizeChanged)
    ) {
  ...
  this.processCalculatedSetting = function (settingName) {
    if (_.isFunction(self.calculatedSettingScripts[settingName])) {
      ...
      try {
        returnValue = self.callValueFunction(
      ...
      if (
        ...
        try {
          self.widgetInstance.onCalculatedValueChanged(
            settingName,
            returnValue,
          );
        } catch (e) {
      ...
      if (settingDef.type == "calculated") {
        ...
        if (!_.isUndefined(script)) {
          ...
          while ((matches = datasourceRegex.exec(script))) {
            ...
            var refreshSettingNames =
              self.datasourceRefreshNotifications[dsName];
  ...
  this.render = function (element) {
    ...
    if (
      !_.isUndefined(self.widgetInstance) &&
      _.isFunction(self.widgetInstance.render)
    ) {
