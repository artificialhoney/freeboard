...
JustGage = function (config) {
  ...
  this.config = {
    ...
    showInnerShadow:
      config.showInnerShadow != null ? config.showInnerShadow : true,
    ...
    levelColorsGradient:
      config.levelColorsGradient != null ? config.levelColorsGradient : true,
    ...
    startAnimationTime: config.startAnimationTime
      ? config.startAnimationTime
      : 700,

    // startAnimationType : string
    // type of initial animation (linear, >, <,  <>, bounce)
    startAnimationType: config.startAnimationType
      ? config.startAnimationType
      : ">",

    // refreshAnimationTime : int
    // length of refresh animation
    refreshAnimationTime: config.refreshAnimationTime
      ? config.refreshAnimationTime
      : 700,

    // refreshAnimationType : string
    // type of refresh animation (linear, >, <,  <>, bounce)
    refreshAnimationType: config.refreshAnimationType
      ? config.refreshAnimationType
      : ">",
  };
  ...
  var canvasW =
    getStyle(document.getElementById(this.config.id), "width").slice(0, -2) * 1;
  var canvasH =
    getStyle(document.getElementById(this.config.id), "height").slice(0, -2) *
    1;
  ...
  var minX =
    dx +
    widgetW / 10 +
    ((widgetW / 6.666666666666667) * this.config.gaugeWidthScale) / 2;
  ...
  this.canvas.customAttributes.pki = function (
    value,
    min,
    max,
    w,
  ...
  // value
  this.txtValue = this.canvas.text(
    this.params.valueX,
    this.params.valueY,
    this.originalValue,
  );
  ...
  // label
  this.txtLabel = this.canvas.text(
    this.params.labelX,
    this.params.labelY,
    this.config.label,
  );
...
JustGage.prototype.generateShadow = function (svg, defs) {
  ...
  if (this.config.showInnerShadow == true) {
    this.canvas.canvas.childNodes[2].setAttribute(
      "filter",
      "url(#" + this.config.id + "-inner-shadow)",
    );
    this.canvas.canvas.childNodes[3].setAttribute(
      "filter",
      "url(#" + this.config.id + "-inner-shadow)",
    );
  }
  ...
  for (var i = 0; i < colors.length; i++) {
    if (pct <= colors[i].pct) {
      if (grad == true) {
        ...
        return (
          "rgb(" +
          [colors[i].color.r, colors[i].color.g, colors[i].color.b].join(",") +
          ")"
        );
      }
    }
